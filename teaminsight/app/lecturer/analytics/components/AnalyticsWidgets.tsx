...
/* --- 1. KPI Cards (Messages & Total Sessions) --- */
export function KpiStats({ qualityData }: { qualityData: QualityData[] }) {
  ...
  qualityData.forEach((d) => {
    if (d.sessionCount > 0) {
      // Summing up total messages (using fallback for safety)
      totalMessagesAll += (d.avgMessages || 0) * d.sessionCount;
      
      // Summing up total sessions count
      totalSessionsAll += d.sessionCount;
    }
  });

  // Calculate global average messages per session
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 w-full">
      ...
      {/* Card 2: Total Sessions (Replaces Avg. Time) */}
      <StatCard 
        icon={<Activity className="text-purple-600" size={24} />} 
        label="Total Sessions" 
        value={totalSessionsAll.toLocaleString()} 
        subtext="Total active conversations"
